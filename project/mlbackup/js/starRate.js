...
function upStars(val) {
    ...
    $(".display_val").val(val.toFixed(1));

    var full = Number.isInteger(val);
    val = parseInt(val);
    var stars = $("#starrate svg");

    stars.slice(0, val).attr("class", "fas fa-star");
    if (!full) {
        stars.slice(val, val + 1).attr("class", "fas fa-star-half-alt");
        val++
    }
    stars.slice(val, 5).attr("class", "far fa-star");
